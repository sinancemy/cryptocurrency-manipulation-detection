...
export default function Dashboard({ user, currentTime, setAutoUpdate }) {  
  ...
  const selectedPricePoint = useMemo(() => {
    ...
    return prices.find(p => p.time === date)
  ...
  const renderDependencies = [user, currentTime, sortByOption, sortOrderOption, prices, postVolume, showPostVolume, 
                                showPostsFromOption, showPostVolume, selectedPoint, selectedPostRange, impactMap, 
                                selectedPriceRange, selectedSources]
