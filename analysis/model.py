...
class CryptoSpeculationModel(nn.Module):
    def __init__(self, device, vocab_size, embed_dim, lstm_hidden_dim, fc_dims, out_dim, batch_size, dropout=0.5):
        ...
        self.lstm = nn.LSTM(embed_dim, lstm_hidden_dim, bidirectional=True, dropout=dropout)
        fc_layers = []
        ...
        self.lstm_hidden = Variable(torch.zeros(2, batch_size, lstm_hidden_dim).to(device))
        self.lstm_cell = Variable(torch.zeros(2, batch_size, lstm_hidden_dim).to(device))
