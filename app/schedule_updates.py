...
# In seconds.
STREAM_UPDATE_TIME = 20
PRICE_UPDATE_TIME = 60
...
def update_stream():
    ...
    requests.post("http://127.0.0.1:5000/stream/update", data={"time": curr_time})
...
def update_prices():
    ...
    curr_time = int(time.time())
    requests.post("http://127.0.0.1:5000/api/update_prices", data={"time": curr_time})
...
def update_posts():
    ...
    curr_time = int(time.time())
    requests.post("http://127.0.0.1:5000/api/update_posts", data={"time": curr_time})
    print("Updated posts")
